...
class CloneHelper:
    ...
    async def link_gen_size(self):
        if self.name is not None:
            ...
            msg = await self.lsg.edit_text(
                f"🤖: {_up} Cloned Succesfully In Drive \n\n<a href='tg://user?id={self.u_id}'>#Cloned To TeamDrive ✅</a>\
                \n\n📀 Info: Calculating...",
    ...
    async def gcl(self):
        self.lsg = await self.mess.reply_text(f"Wait Please | Don't Fuck yourself in high speed 😀")
        destination = f"{DESTINATION_FOLDER}"
        idd = "{" f"{self.g_id}" "}"
        cmd = [
            "/app/gautam/gclone",
