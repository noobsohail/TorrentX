...
async def incoming_message_f(client, message):
    ...
    credit = await message.reply_text(
        f"🧲 ʟᴇᴇᴄʜɪɴɢ ғᴏʀ ʏᴏᴜ<a href='tg://user?id={g_id}'></a>", parse_mode="html"
    )
    i_m_sefg = await message.reply_text("ᴘʀᴏᴄᴇssɪɴɢ...", quote=True)
    ...
    if dl_url is not None:
        ...
        await i_m_sefg.edit_text("🔄 ᴛʀʏɪɴɢ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ ")
...
async def incoming_youtube_dl_f(client, message):
    ...
    current_user_id = message.from_user.id
    credit = await message.reply_text(
        f"<a href='tg://user?id={current_user_id}'>📥 ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ ʏᴏᴜᴛᴜʙᴇ </a>",
        parse_mode="html",
    )
    i_m_sefg = await message.reply_text("🔄 ᴘʀᴏᴄᴇssɪɴɢ", quote=True)
    # LOGGER.info(message)
...
async def g_yt_playlist(client, message):
    ...
    if message.reply_to_message:
        ...
        await message.reply_text("🤦‍♂️ ɴᴏ ᴅᴏᴡɴʟᴏᴀᴅ sᴏᴜʀᴄᴇ ᴘʀᴏᴠɪᴅᴇᴅ", quote=True)
        return
    if "youtube.com/playlist" in url:
        i_m_sefg = await message.reply_text(
            f"<a href='tg://user?id={usr_id}'>📥 ᴅᴏᴡʟᴏᴀᴅɪɴɢ ʏᴏᴜᴛᴜʙᴇ ᴘʟᴀʏʟɪsᴛ</a>",
            parse_mode="html",
        )
        await yt_playlist_downg(message, i_m_sefg, client, is_cloud)

    else:
        await message.reply_text("ʏᴏᴜᴛᴜʙᴇ ᴘʟᴀʏʟɪsᴛ ʟɪɴᴋ ᴏɴʟʏ 😓", quote=True)
...
async def rename_tg_file(client, message):
    ...
    if not message.reply_to_message:
        await message.reply("🤦‍♂️ ɴᴏ ᴅᴏᴡɴʟᴏᴀᴅ sᴏᴜʀᴄᴇ ᴘʀᴏᴠɪᴅᴇᴅ ", quote=True)
        return
    if len(message.command) > 1:
        ...
        try:
            ...
            if message_to_send != "":
                mention_req_user = (
                    f"<a href='tg://user?id={usr_id}'>🤗 ᴅᴏɴᴇ</a>\n\n"
                )
        ...
        await message.reply_text(
            "🖋 ᴘʀᴏᴠɪᴅᴇ ɴᴇᴡ ɴᴀᴍᴇ ᴡɪᴛʜ ᴇxᴛᴇɴsɪᴏɴs", quote=True
        )
